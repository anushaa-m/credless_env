...
class CreditDecisionPipeline:
    def __init__(
        ...
        *,
        ...
        if agent2 is None:
            finetuned = ROOT / "agent2_llm_checkpoints" / "grpo"
            agent2 = Agent2LLMPolicy(model_name_or_path=str(finetuned) if finetuned.exists() else "Qwen/Qwen2.5-0.5B-Instruct")
        self.agent2 = agent2
    ...
    def run(self, user_data: Mapping[str, Any], *, shap_info: Sequence[Mapping[str, Any]] | None = None, env: Any | None = None) -> dict[str, Any]:
        ...
        decision = str(policy_output.decision).upper()
