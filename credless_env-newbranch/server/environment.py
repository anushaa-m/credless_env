...
class CreditAnalystEnvironment(Environment):
    ...
    def step(self, action: str | FinVerseAction) -> Dict[str, Any]:
        ...
        if parsed_action.action_type == "request_info":
            ...
            oracle_label = "APPROVE" if oracle_decision in {"approve", "approved"} else "REJECT"
            decision_label = "APPROVE" if parsed_action.action_type == "approve" else "REJECT"

            oracle_conf = float(self._ground_truth.get("confidence", 0.5))
            oracle_conf = max(0.0, min(1.0, oracle_conf))

            # Simple, stable terminal reward (single source of truth):
            #   + oracle_match (primary signal)
            #   + confidence bonus (optional, via weighting)
            #   - efficiency penalty (info gathering cost)
            oracle_match = 1.0 if decision_label == oracle_label else -1.0
            efficiency_penalty = 0.03 * float(len(self._requested_fields))
            efficiency_penalty += 0.05 if self._market_visible else 0.0
            efficiency_penalty += 0.05 if self._fraud_flags else 0.0

            reward = oracle_match * (0.5 + oracle_conf) - efficiency_penalty
            ...
            self._append_conversation("assistant", f"Final decision: {decision_label}.")
            self._append_conversation(
                "system",
                f"Episode resolved with oracle_decision={oracle_label} (confidence={oracle_conf:.2f}).",
            )
            ...
            self._set_last_info(
                ...
                float(oracle_match),
                {
                    "efficiency": float(efficiency_penalty),
                },
                oracle_decision=oracle_decision,
                oracle_confidence=oracle_conf,
            )
            self._record_reward_components(
                oracle_match=float(oracle_match),
                confidence_bonus=float(oracle_conf),
                efficiency=-float(efficiency_penalty),
            )
            self._mark_last_action_progress(True)
            observation = self._build_observation(
                ...
                episode_score=reward,
                message=explanation.get("explanation", ""),
            )
        else:
