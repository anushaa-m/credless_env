...
def _feature_bounds() -> Dict[str, tuple]:
    """
    Feature min/max bounds used for UI + minor grading helpers.

    Hackathon stability requirement: do NOT hard-fail at import time if the
    training dataset cache is missing. When unavailable, fall back to generic
    (0..1) bounds for normalized features.
    """
    try:
        ...
        return {field: (0.0, 1.0) for field in FEATURE_NAMES}
